import stls from '@/styles/pages/about/MipInfo.module.sass'

export default function MipInfo() {
  return (
    <div className={stls.container}>
      <h1>Институт психологии</h1>
      <ul className={stls.redDot}>
        <li>Наш Институт за современный подход в образовании</li>
        <li>  
          Наша цель - создать комфортную среду обучения и предоставить слушателям самые актуальные и полезные теоретические и практические знания на программах профессиональной переподготовки, повышения квалификации и курсах Life</li>
      </ul>
      <p className={stls.last}>Даем современные практические навыки для открытия частной практики или построения успешной карьеры</p>
    </div>
  )
}
